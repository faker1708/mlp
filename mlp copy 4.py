...
class mlp():
    ...
    def update(self):
        # batch_size 只是在这里用到，用来梯度下降。但和lr同时出现 ，所以感觉没必要有batch_size 这个量了。
    ...
    def train(self,x,y_ref):
        ...
        total_cost = 1000    # 无论如何，会在这么多轮训练后结束，如果不是正数，则此限制无效。
        ...
        while(1):
            ...
            # precious = 14       # print('如果无限循环，考虑降低精度要求。')
            
            # if(fl > ceil_line):
            #     raise(BaseException('loss 太大，有问题.考虑减小学习率'))
            # elif(fl< 2**-precious ):
            #     print('精度达标')
            #     break
            inf =float('inf')
